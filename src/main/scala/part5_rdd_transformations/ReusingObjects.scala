...
/**
 * Avoid recreating JVM objects
 * - Techniques for when performance is critical
 */
...
object ReusingObjects {
   ...
   *
    ...
    Goal: Results should be VERY FAST ---> performance is critical here
   */
  ...
  //////////////////// Version 2 --- uses *mutable* data structures ---- :O
   ...
   * ---- multiple passes over the data are not acceptable (table scans)
   * ---- performance is critical i.e even a few seconds count (real time dashboards.....etc)
   * - Beware of
   * ---- Using tuples - they are (immutable) case classes
   * ---- Converting between Scala collections - especially implicit conversions
   */
