...
object ReadingDAGs {
  ...
  /**
   * Key points:
   * - Find relevant information in the Spark UI
   * - DAG = Directed Acyclic Graph. The visual representation of Spark jobs
   * - Exchanges refer to Shuffles
   * - The number of shuffles is equal to the number of stages
   * - Stages can be clicked on to see their RDD implementations
   *
   */
