...
/**
 * - Optimisation: Partition your data so that Spark doesn't have to
 */
...
object PrePartitioning {
  ...
  // deactivate broadcast joins - i.e Spark will not be able to do HashJoins and Broadcast hashjoins
  ...
  // don't touch this --- To join this two tables Spark would need to repartition these into the same partition schema
  val initialTable = spark.range(1, 10000000).repartition(10) // This is a RoundRobinPartitioning with 10
  val narrowTable = spark.range(1, 5000000).repartition(7) // This is a RoundRobinPartitioning with 7

  // scenario 1
  val wideTable = addColumns(initialTable, 30)
  val join1 = wideTable.join(narrowTable, "id")
  join1.explain()
  // println(join1.count()) // this can take a relatively higher amount of time
  /*
    == Physical Plan ==
  ...
  // scenario 2
  val altNarrow = narrowTable.repartition($"id") // use a HashPartitioner - that is the rows with this ID column will sit on the same partition
  val altInitial = initialTable.repartition($"id")
  // join on co-partitioned DFs --- which is faster
  val join2 = altInitial.join(altNarrow, "id")
  val result2 = addColumns(join2, 30)
  result2.explain()
  // println(result2.count()) // faster than scenario 1 for a similar joiin
  ...
  /*
    ...
    *(5) Project [id#0L, (id#0L * 1) AS newCol1#105L, (id#0L * 2) AS newCol2#106L, (id#0L * 3) AS newCol3#107L, (id#0L * 4) AS newCol4#108L, (id#0L * 5) AS newCol5#109L, (id#0L * 6) AS newCol6#110L, (id#0L * 7) AS newCol7#111L, (id#0L * 8) AS newCol8#112L, (id#0L * 9) AS newCol9#113L, (id#0L * 10) AS newCol10#114L, (id#0L * 11) AS newCol11#115L, (id#0L * 12) AS newCol12#116L, (id#0L * 13) AS newCol13#117L, (id#0L * 14) AS newCol14#118L, (id#0L * 15) AS newCol15#119L, (id#0L * 16) AS newCol16#120L, (id#0L * 17) AS newCol17#121L, (id#0L * 18) AS newCol18#122L, (id#0L * 19) AS newCol19#123L, (id#0L * 20) AS newCol20#124L, (id#0L * 21) AS newCol21#125L, (id#0L * 22) AS newCol22#126L, (id#0L * 23) AS newCol23#127L, ... 7 more fields]
    +- *(5) SortMergeJoin [id#0L], [id#4L], Inner
       ...
       :  +- Exchange hashpartitioning(id#0L, 200), false, [id=#91] //// This is the right type of partitioner for better performance
       :     +- *(1) Range (1, 10000000, step=1, splits=1)
  ...
  /**
   * Important point: partition early.
   * Partitioning late is at best what Spark naturally does.
   *
   * Whenever Spark does not know the partitioning schemes for the data frames that are about to be joined, it forces them into a hash partitioning
   */
  ...
  // println(result3.count()) // this can take a number of seconds
  result3.explain()

  /**
   * Notice the two shuffles on the bigger dataframe
   */
  ...
  /*
    ...
    *(6) Project [id#0L, newCol1#166L, newCol2#167L, newCol3#168L, newCol4#169L, newCol5#170L, newCol6#171L, newCol7#172L, newCol8#173L, newCol9#174L, newCol10#175L, newCol11#176L, newCol12#177L, newCol13#178L, newCol14#179L, newCol15#180L, newCol16#181L, newCol17#182L, newCol18#183L, newCol19#184L, newCol20#185L, newCol21#186L, newCol22#187L, newCol23#188L, ... 7 more fields]
    +- *(6) SortMergeJoin [id#0L], [id#4L], Inner // <---- This step is a standard join ---- if broadcasting was enabled this would be a BroadcastHashJoin
       :- *(3) Sort [id#0L ASC NULLS FIRST], false, 0
       :  +- Exchange hashpartitioning(id#0L, 200), false, [id=#154] //Paritioning step since the RoundRobinParitioning does not guarantee that it has been partitioned correctly
  ...
  /**
   * TODO Exercise: what would happen if we just repartitioned the smaller table to 10 partitions?
   * This idea is not a good one.......TERRIBLE!
   *
   * Why?
   * - Perform more shuffles
   * - Neither repartitioning guarantees that all the rows with the same ID are on the same partition, which is a mandatory guarantee that must be held before the join takes place.
   *
   */

  initialTable.join(narrowTable.repartition(10), "id").explain() // When viewing the query plan this is identical to scenario 1 ---> that is we have 4 shuffles. 2 on each data from

  /**
   * Key points:
   * - Parition your data early so that Spark doesn't have to
   * --- Make the joined DFs share the same partitioner. E.g partition by the same columns
   * --- Decorate the joined DF later (especially if you have lots of transformations)
   * - Partitioning late is bad
   * --- At best it is the same performance as Spark out-of-the-box
   * --- At worst it can lead to worse performance than not partitioning at all
   */
